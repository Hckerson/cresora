...
export default function HeroSection() {
  return (
    <div className="div grid grid-cols-1 gap-y-[72px] px-6 pt-[93px] md:px-12">
      <div className="flex w-full flex-col lg:flex-row">
        <div className="flex flex-1 flex-col gap-y-[24px]">
          ...
          >
            ...
            >
              Built for founders, operators, and teams to stay on top of cash
              flow, budgets, and runway in real time
            </p>
          </div>
        </div>
        <div className="flexy xs:items-end xs:flex-row box-border w-full flex-col justify-between pt-12 lg:w-fit lg:flex-col lg:justify-end lg:pt-0">
          <Button size="sm" style="hover:bg-blue-600" isLoading={false}>
            Start free trial
          </Button>
          <div className="flexy justify-between gap-x-4">
            <p className="leading-1.1 text-base text-[#B6B7B8]">
              Trusted by{" "}
              <span className="block text-[#030E1F]">1,200+ teams</span>
            </p>
            <span className="flex items-center -space-x-2">
              {roundedImages.map((image, idx) => (
                <div key={idx} className="shrink-0 rounded-full">
                  <Image
                    height={100}
                    width={100}
                    alt="Pinera icon"
